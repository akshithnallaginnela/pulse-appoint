      ...
      <div className="container flex h-16 items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {isAuthenticated && user?.role === 'doctor' && (
            <Link to="/doctor/dashboard" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-base">
              Dashboard
            </Link>
          )}
          {isAuthenticated && user?.role === 'doctor' && (
            <Link to="/doctor/appointments" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-base">
              My Appointments
            </Link>
          )}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <div className="px-2 py-1.5">
                  <p className="text-sm font-medium">{user?.role === 'doctor' ? 'Dr. ' : ''}{user?.firstName} {user?.lastName}</p>
                ...
                <DropdownMenuItem onClick={() => navigate(user?.role === 'doctor' ? "/doctor/appointments" : "/appointments")}>
